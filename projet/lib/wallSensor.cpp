...
void WallSensor::avoid2Obstacles(uint16_t pathPosition)
{
    ...
    case 2:
        if (obstaclePosition == NO_OBSTACLE)
        {
            ...
            // Nous utilisons un timer afin de le forcer a avancer jusqu'à ce que son centre de rotation
            // soit vis-a-vis K et L, ce processus prends environ 2.3 secondes
            motors.givePower();
...
void WallSensor::avoid3Obstacles(uint16_t pathPosition)
{
    ...
    case 1:
        ...
        if (obstaclePosition == CLOSE)
        {
            // obstacle au point K
            // Nous utilisons un timer afin de le forcer a avancer jusqu'à ce que son centre de rotation
            // soit vis-a-vis K et L, ce processus prends environ 2.4 secondes
            motors.givePower();
    ...
    case 2:
        ...
        if (obstaclePosition == CLOSE)
        {
            // obstacle au point H ou F
            motors.givePower();
            motors.turnRightSlight(); // on doit le forcer a aller au chemin sans obstacle
            _delay_ms(1000);
            lineSensor.followLineObstacles();
            motors.stop();
            _delay_ms(1000);
        ...
        else if (obstaclePosition == FAR)
        {
            // obstacle au point I ou G
            motors.givePower();
            motors.turnLeftSlight(); // on doit le forcer a aller au chemin sans obstacle
            _delay_ms(1000);
            lineSensor.followLineObstacles();
            motors.stop();
            _delay_ms(1000);
